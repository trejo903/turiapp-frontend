...
export default function EditarPerfil() {
  ...
  const params = useLocalSearchParams<{ nombre?: string; apellido?: string; email?: string }>();
  ...
  const {
    ...
    defaultValues: {
      ...
      correo: String(params?.email ?? ""),
    },
  });

  const onSubmit = (values: FormValues) => {
    // Aquí iría tu fetch al backend si lo necesitas
    Alert.alert("Perfil actualizado", `${values.nombre} ${values.apellido}`);
    reset(values);
    router.back();
  };
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.select({ ios: "padding" })}>
      ...
      >
        ...
        <View style={{ gap: 10, marginTop: 8 }}>
          ...
          <Pressable onPress={() => router.back()} style={({ pressed }) => [styles.buttonGhost, pressed && { opacity: 0.8 }]}>
            <Text style={[styles.buttonText, { color: "#111827" }]}>Cancelar</Text>
          </Pressable>
        </View>
      </ScrollView>
    </KeyboardAvoidingView>
